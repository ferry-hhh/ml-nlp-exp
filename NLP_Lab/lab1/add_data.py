...
'''
@File   :   add_data.py
@Time   :   2021/11/12 23:40:17
@Author :   WYJ 
@Desc   :   增加更多的训练集，完善三大概率矩阵
'''
...
def my_filter(file_path):
    ...
    for line in open(file_path, encoding="utf-8"):
        line = line.replace(',', '，')
        new_line = re.sub(punctuations, "", line)
        if not new_line.isspace():
            f1.write(new_line)
    for line in open(raw_data_path + r"\lab1_data\article\sentence.txt", encoding="utf-8"):
        line = line.replace(',', '，')
        new_line = re.sub(punctuations, "", line)
        if not new_line.isspace():
            f1.write(new_line)
    for line in open(raw_data_path + r"\lab1_data\article\news.txt", encoding="utf-8"):
        line = line.replace(',', '，')
        new_line = re.sub(punctuations, "", line)
        if not new_line.isspace():
            f1.write(new_line)
    for line in open(raw_data_path + r"\lab1_data\article\边城.txt", encoding="utf-8"):
        line = line.replace(',', '，')
        new_line = re.sub(punctuations, "", line)
        if not new_line.isspace():
            f1.write(new_line)
    for line in open(raw_data_path + r"\lab1_data\article\四世同堂.txt", encoding="utf-8"):
        line = line.replace(',', '，')
        new_line = re.sub(punctuations, "", line)
        if not new_line.isspace():
            f1.write(new_line)
    for line in open(raw_data_path + r"\lab1_data\article\百年孤独.txt", encoding="utf-8"):
        line = line.replace(',', '，')
        new_line = re.sub(punctuations, "", line)
        if not new_line.isspace():
            f1.write(new_line)
    for line in open(raw_data_path + r"\lab1_data\article\唐诗三百首.txt", encoding="utf-8"):
        line = line.replace(',', '，')
        new_line = re.sub(punctuations, "", line)
        if not new_line.isspace():
            f1.write(new_line)
    for line in open(raw_data_path + r"\lab1_data\article\呐喊.txt", encoding="utf-8"):
        line = line.replace(',', '，')
        new_line = re.sub(punctuations, "", line)
        if not new_line.isspace():
            f1.write(new_line)
    for line in open(raw_data_path + r"\lab1_data\article\简爱.txt", encoding="utf-8"):
        line = line.replace(',', '，')
        new_line = re.sub(punctuations, "", line)
        if not new_line.isspace():
            f1.write(new_line)
...
def build_init(sens_list):
    ...
    for key in init_prob.keys():
        init_prob[key] = math.log(init_prob.get(key) / total) # 计算并用log函数规范化是概率
